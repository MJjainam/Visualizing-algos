...
function Bellman_Ford(){
	if(validate_inputs() == "Source"){
		...
		{	
			...
			{
				var e = Graph.Edges[j];
				var u = get_vertex(e.origin.id);
				var v = get_vertex(e.endpoint.id);
				relax(u,v);
			}
		}	
...
function show_distances(){
	alert("Showing weights");
	var i;
	for(i=0; i < Graph.Vertices.length; i++)
	{
		alert("Vertex : " + Graph.Vertices[i].element.id + " Distance : " + Graph.Vertices[i].distance);
	}
}
